...
import pdf2image  # если не установлена, будет ошибка, но мы обработаем
...
# --- ЗАГРУЗКА ПЛАНИРОВКИ ---
st.markdown("### 🖼️ Загрузить планировку (для визуальной справки)")
uploaded_file = st.file_uploader(
    "Выберите файл (PNG, JPG, PDF)",
    type=["png", "jpg", "jpeg", "pdf"],
    label_visibility="collapsed"
)
if uploaded_file:
    try:
        # Попытка отобразить как изображение (если PDF, конвертируем первую страницу)
        if uploaded_file.type == "application/pdf":
            try:
                images = pdf2image.convert_from_bytes(uploaded_file.read(), first_page=1, last_page=1)
                st.image(images[0], caption="Загруженная планировка (первая страница)", use_container_width=True)
            except Exception:
                st.info("📄 PDF загружен. Для просмотра установите библиотеку pdf2image (pip install pdf2image).")
                st.download_button("Скачать PDF", data=uploaded_file.getvalue(), file_name="plan.pdf", mime="application/pdf")
        else:
            st.image(uploaded_file, caption="Загруженная планировка", use_container_width=True)
    except Exception as e:
        st.warning(f"Не удалось отобразить файл: {e}")

# --- НЮАНСЫ (ЧЕКБОКСЫ) ---
st.markdown("**🔧 Особенности помещения (дополнительные параметры)**")
...
- Подвесной потолок: {'Да' if suspended_ceiling else 'Нет'}
- Балки на потолке: {'Да (высота ' + str(beam_height) + ' мм)' if beams else 'Нет'}
- Окна на солнечную сторону: {'Да' if sun_side else 'Нет'}
- Защита кабеля гофрой: {'Да' if cable_protection else 'Нет'}
- Гипсокартонные стены: {'Да' if gypsum_walls else 'Нет'}
- Проход кабеля через стены: {'Да' if through_walls else 'Нет'}

Правила расчёта количества оборудования:
- Видеокамеры: 1 камера на 20 м² (минимум 2). Расчёт: площадь / 20, округление вверх до целого, но не менее 2.
- Дымовые извещатели: 1 на 30 м² (минимум 2 на помещение). Расчёт: площадь / 30, округление вверх до целого, но не менее 2.
- Ручные извещатели: 1 на каждый выход (если дверей > 0).
- Датчики движения: 1 на 20 м² (минимум 1).
- СКУД: 1 считыватель на дверь, 1 контроллер на помещение.
- Длина кабеля (для сметы): периметр × 1.5 (с запасом).
При наличии подвесного потолка, балок, солнечной стороны — учти дополнительные требования (установка за потолком, защита от засветки, выбор камер с WDR).
При выборе оборудования строго придерживайся целевого перечня. В спецификации указывай конкретные модели.
...
3. Требования к системам:
   - САПС: оборудование (из целевого перечня), количество извещателей с расчётом, логика срабатывания.
   - СОУЭ: тип оповещения (речевое), количество оповещателей.
   - СКУД: количество считывателей, контроллеров, биотерминалов, интеграция с САПС (разблокировка по пожару).
   - СОТС: датчики на двери/окна, объёмные датчики.
   - СОТ: количество камер с расчётом, разрешение, ИК-подсветка, архив 30 суток.
...
В смете укажи цены на оборудование из целевого перечня (примерные рыночные).
"""

                    elif scenario == "Смета":
                        prompt = base_prompt + """
Сформируй смету на оснащение помещения. Включи таблицу с колонками: Наименование (с указанием модели из целевого перечня), Кол-во, Цена за ед. (руб.), Сумма (руб.). Добавь итоговую сумму (оборудование + монтаж 30% + накладные 15%). Покажи расчёт количества оборудования (например, площадь / 20 = X → минимум Y камер).
"""

                    elif scenario == "Проект":
                        prompt = base_prompt + """
Сформируй полный проект:
